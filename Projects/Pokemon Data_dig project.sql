...
SELECT * From pokemon;
--What are the top 10 beefiest pokemon?
SELECT Name, HP FROM pokemon order by HP desc limit 10;

--Who is the the weakest in HP? 
SELECT name, Min(HP) from pokemon;

--Who are the highest attatckers?
SELECT name, Attack From pokemon order by Attack desc limit 10;


--Who are the highest defenders?
SELECT name, Defense from pokemon order by defense desc limit 10;

--Average attack?
select AVG(Attack) as average_attack from pokemon order by Attack desc limit 10;

--Grouping the average attack
SELECT Type_1, AVG(Attack) as total_attack from pokemon group by type_1 order by total_attack desc;

--Grouping the average SP.attack
SELECT Type_1, AVG(Sp_Atk) as total_Sp_atk from pokemon group by type_1 order by total_Sp_atk desc;

/**COUNT**/
select COUNT(*) from pokemon where Type_1 =  "Fire";
select COUNT(*) from pokemon where Type_1 =  "Dragon";
select COUNT(*) from pokemon where Type_1 =  "Water";
select COUNT(*) from pokemon where Type_1 =  "Grass";
select COUNT(*) from pokemon where Type_1 =  "Ghost";
select COUNT(*) from pokemon where Type_1 =  "Bug";
select COUNT(*) from pokemon where Type_1 =  "Rock";
select COUNT(*) from pokemon where Type_1 =  "Normal";

--Using 'CASE' to show me how many pokemon are in which type.
select COUNT(*), 
    CASE
    When Type_1 = "Fire" THEN "Fire"
    When Type_1 = "Normal" THEN "Normal"
    When Type_1 = "Rock" then "Rock" 
    When Type_1 = "Ghost" Then "Ghost" 
    When Type_1 = "Psychic" Then "Psychic"
    When Type_1 = "Electric" Then "Elecrtic" 
    When Type_1 =  "Ground" Then "Ground"
    When Type_1 = "Bug" Then "Bug"
    When Type_1 = "Fairy" Then "Fairy" 
    When Type_1 = "Poison" Then "Poison"
    When Type_1 = "Water" Then "Water"
    When Type_1 = "Dragon" Then "Dragon"
    When Type_1 = "Grass" Then "Grass" 
    When Type_1 = "Ice" Then "Ice" 
    When Type_1 = "Fighting" Then "Fighting"
    End as "pokemon_types"
